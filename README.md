...
> 💡 **小撇步**：除了關鍵字觸發，機器人還具備 **AI 意圖識別**能力——即使你用自然語言詢問如「這個安全嗎？ https://xxx.shop」，AI 也能判斷意圖並自動觸發正確模式。
...
### 🤖 AI 意圖識別 (Intent Detection)

純關鍵字比對有其侷限，無法涵蓋全部自然語言表達。當靜態比對未命中，但訊息含有連結時，系統會自動呼叫 **Gemini Flash-Lite** 對訊息進行意圖分類，擴大各模式的觸發範圍。

| 匹配方式 | 時機 | 表現 |
| :--- | :--- | :--- |
| 靜態關鍵字 | 第一層，零延遲 | `大綱 https://...` 直接觸發 |
| AI 意圖識別 | 第二層，經 API 判斷 | `這個安全嗎？ https://...` 仍可正確識別 |
...
1. **🔍 網域分析**：TLD 類型、品牌仿冒與網域亂碼程度。
2. **⚠️ URL 結構風險**：指出可疑路徑或高風險參數組合。
3. **🛡️ 風險評級**：整合靜態評分與 AI 判斷，給出🔴/🟡/🟢 明確評級及建議行動。
