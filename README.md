...
本專案為部署於 **Google Apps Script (GAS)** 的高效能 **AI 資訊查核助手**，專為 LINE 群組設計。具備三大核心功能：**「數位內容鑑識」**、**「影片精華整理」**與全新加入的 **「詐騙網址偵測」**。能精準判別影片真偽、識別 AI 生成內容，並能即時鑑別可疑詐騙連結，防堡群組成員受到釣魚攻擊。
...
| **🔍 詐騙網址偵測** | `詐騙`, `釣魚`, `可疑`, `偵測`, `網址查核`, `安全嗎`, `安不安全`, `有沒有詐騙` | 静態風險評分（TLD、參數、仿冒品牌）包裹 AI 深度研判，判定是否為詐騙連結。 |
| **指令查詢** | `指令查詢`, `幫助`, `help`, `/help` | 顯示目前的指令表與使用範例。 |

> 💡 **小撇步**：只要訊息開頭包含上述任一關鍵字，後接 YouTube 連結即可觸發。例如：`大綱 https://...`
> 💡 **小撇步**：除了關鍵字觸發，機器人還具備 **AI 意圖識別**能力——即使你用自然語詢問請如「這個安全嗎？ https://xxx.shop」，AI 也能判斷意圖並自動觸發正確模式。
...
純關鍵字比對有其穷所，無法涵蓋全部自然語言表達。當靜態比對未命中，但訊息含有連結時，系統會自動呼叫 **Gemini Flash-Lite** 對訊息進行意圖分類，擴大各模式的觸發範圍。
...
- **v2026.05.02.01**:
  ...
  - 🤖 **新增 AI 意圖識別機制**：靜態關鍵字未命中時，自動呼叫 Gemini 辺分意圖，支援自然語言訢關如「這個安全嗎」。
  - 📊 **指令表更新**：整合三模式操作說明與範例。
- **v2026.05.01.01**:
