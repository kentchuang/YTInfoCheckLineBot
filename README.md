...
- **v2026.05.07.01**:
  - ⚖️ **優化事實查核提示詞 (System Instruction)**
    * 1. 強化對「行銷包裝」與「內容實質」的辨識：避免 AI 僅因標題誇張或行銷手段而誤判專業內容為高風險。
    * 2. 提升來源身分核實邏輯：優先檢查作者是否為具名專業人士，區分專家頻道與匿名農場。
    * 3. 優化 AI 參與度評估：更精確地描述製作型態（專家實拍/知識分享/內容農場）。
    * 4. 角色定位調整：升級為具備高度資訊素養的資深數位內容鑑識專家。
